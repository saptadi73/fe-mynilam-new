<template>
  <div class="bg-image-wave2 px-5 md:px-16 pb-4">
    <BaseHeaderTitle :title="'Daftar Agen/Koperasi ' + daerah" />
    <div class="bg-[#F6FDFF] p-4 rounded-3xl border border-[#015438]">
      <div class="flex flex-col md:flex-row gap-y-2 md:gap-y-0 md:gap-x-5 justify-start">
        <div class="flex flex-col md:flex-row gap-y-2 md:gap-y-0 md:gap-x-2">
          <BaseSearchBar v-model="searchValue" placeholder="Cari nama"></BaseSearchBar>
      ...
      <div class="grid grid-cols-12 gap-y-4 md:gap-y-4 md:gap-x-4 mt-2">
        ...
        <div
          ...
          v-if="!daftarPenjualan.data.value && !daftarPenjualan.isLoading.value"
        >
          Tidak ada data untuk ditampilkan
        </div>
        <BaseSkeletonCard
          v-if="daftarPenjualan.isLoading.value"
          v-for="n in 3"
          :key="n"
          class="col-span-12 md:col-span-6 lg:col-span-3"
        />
        <BaseCard
          v-for="data in daftarPenjualan.data.value"
          :key="data.id"
...
const params = ref<DaftarPenjualanParams>({})
...
const handleParamValue = () => {
  ...
  if (selectedKabupaten)
    params.value = { kabupaten_id: selectedKabupaten.id, destination_actor_name: searchValue.value }
}
